...
export class RteModalComponent {
  ...
  content = '';
  constructor(private richTextEditor: RichTextEditorService) {}
  ngOnInit() {
  ...
  injectSignature() {
    const html = '${Dynamic Inline Content}';
    const formatted = `<span class="rte-variable-token" contenteditable="false">${html}</span>`;

    this.richTextEditor.insertContentAtCursor(formatted);
  }
  injectSignature2() {
